...
import { X, AlertCircle, Calendar, Mail, Phone } from 'lucide-react'

interface TrialGracePeriodModalProps {
  isOpen: boolean
  onClose: () => void
  trialEndDate: string
}

export function TrialGracePeriodModal({ 
  isOpen, 
  onClose, 
  trialEndDate
}: TrialGracePeriodModalProps) {
  const [dismissed, setDismissed] = useState(false)

  useEffect(() => {
    if (isOpen) {
      // Check if user has dismissed this modal before
      const dismissedKey = `trial-grace-dismissed-${trialEndDate}`
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="relative w-full max-w-md rounded-lg bg-card p-6 shadow-xl border border-border">
        ...
        <p className="mb-4 text-foreground">
          Your trial period has expired. Please confirm your admission to avoid service interruption.
        </p>
