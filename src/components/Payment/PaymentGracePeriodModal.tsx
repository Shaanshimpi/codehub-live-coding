...
import { X, AlertCircle, CreditCard, Mail, Phone } from 'lucide-react'

interface PaymentGracePeriodModalProps {
  isOpen: boolean
  onClose: () => void
  nextInstallment: {
    dueDate: string
...
export function PaymentGracePeriodModal({ 
  isOpen, 
  onClose, 
  nextInstallment, 
  daysRemaining 
}: PaymentGracePeriodModalProps) {
  const [dismissed, setDismissed] = useState(false)

  useEffect(() => {
    if (isOpen) {
      // Check if user has dismissed this modal before
      const dismissedKey = `payment-grace-dismissed-${nextInstallment.dueDate}`
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="relative w-full max-w-md rounded-lg bg-card p-6 shadow-xl border border-border">
        ...
        <p className="mb-4 text-foreground">
          Your payment is overdue. Please make sure to complete the payment to avoid service interruption.
        </p>

        <div className="mb-4 rounded-lg border border-border bg-muted/50 p-4">
          <div className="mb-2 flex items-center gap-2">
            <CreditCard className="h-5 w-5 text-muted-foreground" />
            <h3 className="font-semibold text-foreground">Installment Details</h3>
          </div>
