...
interface SessionWorkspaceLayoutProps {
  sessionCode?: string
  sessionTitle?: string
  onFileChange?: (fileId: string | null, fileName: string) => void
  showTrainerCode?: boolean
...
export function SessionWorkspaceLayout({
  ...
  useEffect(() => {
    if (selectedFile) {
      ...
      if (onFileChange) {
        onFileChange(selectedFile.id, selectedFile.name)
      }
    } else {
      // No file selected
      if (onFileChange) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {showAI && selectedFile && (
          <div className="w-[35%] border-l bg-muted/30 p-2">
            <AIAssistantPanel
              role="student"
              lectureId={sessionCode || 'workspace'}
              language={language}
              code={code}
